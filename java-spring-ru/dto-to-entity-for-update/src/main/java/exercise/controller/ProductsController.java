...
public class ProductsController {
    ...
    // BEGIN
    @PutMapping(path = "/{id}")
    public ProductDTO update(@RequestBody ProductUpdateDTO productDTO, @PathVariable Long id) {
        var product = productRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Product with id " + id + " not found"));
        toEntityUpdate(productDTO, product);
        productRepository.save(product);
        var productDto = toDTO(product);
        return productDto;
    }

    private void toEntityUpdate(ProductUpdateDTO productDTO, Product product) {
